...
public:
    ...
    void handleAuthLogin(const mw::HTTPServer::Request& req,
                         mw::HTTPServer::Response& res);

    /// Handles the OIDC callback.
    /// Exchanges the code for tokens, creates a session, or redirects to
    /// username setup.
    void handleAuthCallback(const mw::HTTPServer::Request& req,
                            mw::HTTPServer::Response& res);

    /// Displays the username setup form.
    /// Required for first-time OIDC logins.
    void handleAuthSetupUsername(const mw::HTTPServer::Request& req,
                                 mw::HTTPServer::Response& res);

    /// Processes the username setup form submission.
    /// Creates the local user account and session.
    void handleAuthSetupUsernamePost(const mw::HTTPServer::Request& req,
                                     mw::HTTPServer::Response& res);

    /// Logs the user out.
    /// Destroys the current session and clears cookies.
    void handleAuthLogout(const mw::HTTPServer::Request& req,
                          mw::HTTPServer::Response& res);
...
private:
    ...
    getCurrentSession(const mw::HTTPServer::Request& req);
    bool checkCSRF(const mw::HTTPServer::Request& req);
    mw::E<void> processActivity(const nlohmann::json& activity,
                                const std::string& sender_id);
    mw::E<void> handleCreate(const nlohmann::json& activity,
                             const std::string& sender_id);
    mw::E<void> handleFollow(const nlohmann::json& activity,
                             const std::string& sender_id);
    mw::E<void> handleAccept(const nlohmann::json& activity,
                             const std::string& sender_id);
    mw::E<int64_t> ensureRemoteUser(const std::string& uri);
    mw::E<void> createPost(const User& author, const std::string& content);
    mw::E<std::string> handleUpload(const mw::HTTPServer::Request& req,
                                    const User& uploader);
    mw::E<std::optional<User>> resolveRemoteUser(const std::string& username,
    ...
    mw::E<nlohmann::json> fetchRemoteActor(const std::string& uri,
