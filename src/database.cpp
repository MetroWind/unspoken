...
mw::E<int64_t> Database::createUser(const User& user)
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "INSERT INTO users (username, display_name, bio, email, uri, public_key, private_key, host, created_at, avatar_path) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"));
...
static User rowToUser(const std::tuple<int64_t, std::string, std::string, std::string, std::string, std::string, std::string, std::string, std::string, int64_t, std::string>& row)
{
    User u;
    u.id = std::get<0>(row);
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("SELECT id, username, display_name, bio, email, uri, public_key, private_key, host, created_at, avatar_path FROM users WHERE id = ?;"));
    DO_OR_RETURN(stmt.bind(id));
    ASSIGN_OR_RETURN(auto rows, (db->eval<int64_t, std::string, std::string, std::string, std::string, std::string, std::string, std::string, std::string, int64_t, std::string>(std::move(stmt))));
    
    if (rows.empty()) return std::nullopt;
    return rowToUser(rows[0]);
}

mw::E<std::optional<User>> Database::getUserByUsername(const std::string& username)
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("SELECT id, username, display_name, bio, email, uri, public_key, private_key, host, created_at, avatar_path FROM users WHERE username = ?;"));
    DO_OR_RETURN(stmt.bind(username));
    ASSIGN_OR_RETURN(auto rows, (db->eval<int64_t, std::string, std::string, std::string, std::string, std::string, std::string, std::string, std::string, int64_t, std::string>(std::move(stmt))));
    if (rows.empty()) return std::nullopt;
    return rowToUser(rows[0]);
}
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("SELECT id, username, display_name, bio, email, uri, public_key, private_key, host, created_at, avatar_path FROM users WHERE uri = ?;"));
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "INSERT INTO posts (uri, author_id, content_html, content_source, in_reply_to_uri, visibility, created_at, is_local) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?);"));
...
static Post rowToPost(const std::tuple<int64_t, std::string, int64_t, std::string, std::string, std::string, int, int64_t, int>& row)
{
    Post p;
    p.id = std::get<0>(row);
    p.uri = std::get<1>(row);
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("SELECT id, uri, author_id, content_html, content_source, in_reply_to_uri, visibility, created_at, is_local FROM posts WHERE id = ?;"));
    DO_OR_RETURN(stmt.bind(id));
    ASSIGN_OR_RETURN(auto rows, (db->eval<int64_t, std::string, int64_t, std::string, std::string, std::string, int, int64_t, int>(std::move(stmt))));
    if (rows.empty()) return std::nullopt;
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("SELECT id, uri, author_id, content_html, content_source, in_reply_to_uri, visibility, created_at, is_local FROM posts WHERE uri = ?;"));
    DO_OR_RETURN(stmt.bind(uri));
    ASSIGN_OR_RETURN(auto rows, (db->eval<int64_t, std::string, int64_t, std::string, std::string, std::string, int, int64_t, int>(std::move(stmt))));
    if (rows.empty()) return std::nullopt;
    return rowToPost(rows[0]);
...
mw::E<std::vector<Post>> Database::getTimeline(int64_t user_id, int limit, int offset)
{
    // Home timeline: posts from followed users + own posts
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "SELECT p.id, p.uri, p.author_id, p.content_html, p.content_source, p.in_reply_to_uri, p.visibility, p.created_at, p.is_local "
        "FROM posts p "
        "WHERE p.author_id = ? OR p.author_id IN (SELECT target_id FROM follows WHERE follower_id = ? AND status = 1) "
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "SELECT id, uri, author_id, content_html, content_source, in_reply_to_uri, visibility, created_at, is_local "
        "FROM posts WHERE visibility = 0 ORDER BY created_at DESC LIMIT ? OFFSET ?;"));
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "INSERT OR REPLACE INTO follows (follower_id, target_id, status, uri) VALUES (?, ?, ?, ?);"));
    DO_OR_RETURN(stmt.bind(follow.follower_id, follow.target_id, follow.status, follow.uri));
    return db->execute(std::move(stmt));
}

mw::E<void> Database::updateFollowStatus(int64_t follower_id, int64_t target_id, int status)
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "UPDATE follows SET status = ? WHERE follower_id = ? AND target_id = ?;"));
    DO_OR_RETURN(stmt.bind(status, follower_id, target_id));
    return db->execute(std::move(stmt));
}

mw::E<std::optional<Follow>> Database::getFollow(int64_t follower_id, int64_t target_id)
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("SELECT follower_id, target_id, status, uri FROM follows WHERE follower_id = ? AND target_id = ?;"));
    DO_OR_RETURN(stmt.bind(follower_id, target_id));
    ASSIGN_OR_RETURN(auto rows, (db->eval<int64_t, int64_t, int, std::string>(std::move(stmt))));
    if (rows.empty()) return std::nullopt;
    Follow f;
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "INSERT INTO jobs (type, payload, attempts, next_try, status) VALUES (?, ?, ?, ?, ?);"));
    DO_OR_RETURN(stmt.bind(job.type, job.payload, job.attempts, job.next_try, job.status));
    DO_OR_RETURN(db->execute(std::move(stmt)));
    return db->lastInsertRowID();
}
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "SELECT id, type, payload, attempts, next_try, status FROM jobs WHERE status = 0 AND next_try <= ? LIMIT ?;"));
    DO_OR_RETURN(stmt.bind(mw::timeToSeconds(mw::Clock::now()), limit));
    ASSIGN_OR_RETURN(auto rows, (db->eval<int64_t, std::string, std::string, int, int64_t, int>(std::move(stmt))));
...
mw::E<void> Database::updateJob(int64_t id, int status, int attempts, int64_t next_try)
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr(
        "UPDATE jobs SET status = ?, attempts = ?, next_try = ? WHERE id = ?;"));
...
{
    ASSIGN_OR_RETURN(auto stmt, db->statementFromStr("DELETE FROM jobs WHERE id = ?;"));
    DO_OR_RETURN(stmt.bind(id));
    return db->execute(std::move(stmt));
}
