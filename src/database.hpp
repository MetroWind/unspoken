...
public:
    ...
    mw::E<std::vector<Post>> getTimeline(int64_t user_id, int limit, int offset);
    mw::E<std::vector<Post>> getPublicTimeline(int limit, int offset);

    // Follow DAO
    mw::E<void> createFollow(const Follow& follow);
    mw::E<void> updateFollowStatus(int64_t follower_id, int64_t target_id, int status);
    mw::E<std::optional<Follow>> getFollow(int64_t follower_id, int64_t target_id);

    // Job DAO
    mw::E<int64_t> enqueueJob(const Job& job);
    mw::E<std::vector<Job>> getPendingJobs(int limit);
    mw::E<void> updateJob(int64_t id, int status, int attempts, int64_t next_try);
