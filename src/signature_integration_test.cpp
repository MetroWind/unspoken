...
protected:
    void SetUp() override
    {
        ...
        verifier = std::make_unique<SignatureVerifier>(
            std::move(h), std::make_unique<mw::Crypto>(), std::move(d),
            system_actor_uri);
    }
...
TEST_F(SignatureIntegrationTest, IncomingSignatureVerification)
{
    ...
    EXPECT_CALL(*db_mock, getUserByUri(alice_uri))
        .WillRepeatedly(Return(alice));
    ...
    std::string sig_header = "keyId=\"" + alice_uri +
                             "#main-key\",algorithm=\"hs2019\",headers=\"("
    ...
    EXPECT_CALL(*h2, post(_))
        .WillOnce(Invoke(
            [&](const mw::HTTPRequest& req) -> mw::E<const mw::HTTPResponse*>
            {
                ...
                EXPECT_TRUE(sig.find("keyId=\"" + alice_uri + "#main-key\"") !=
                            std::string::npos);
                EXPECT_TRUE(sig.find("algorithm=\"hs2019\"") !=
                            std::string::npos);

                size_t sig_start = sig.find("signature=\"") + 11;
                size_t sig_end = sig.find("\"", sig_start);
                std::string sig_b64 =
                    sig.substr(sig_start, sig_end - sig_start);
                ...
                auto valid = crypto->verifySignature(
                    mw::SignatureAlgorithm::RSA_V1_5_SHA256, keys.public_key,
                    sig_bytes, to_verify);
