...
SignatureVerifier::SignatureVerifier(std::unique_ptr<mw::HTTPSessionInterface> http_client,
    ...
    : http_client(std::move(http_client)), crypto(std::move(crypto)), db(std::move(db)), system_actor_uri(system_actor_uri)
{
}
...
mw::E<std::string> SignatureVerifier::fetchAndCacheActor(const std::string& uri)
{
    ...
    if (!system_actor_res || !system_actor_res.value()) return std::unexpected(mw::runtimeError("System actor not found"));
    auto sys_actor = *system_actor_res.value();
    if (!sys_actor.private_key) return std::unexpected(mw::runtimeError("System actor has no private key"));
    ...
    try
    {
        ...
        if (public_key_pem.empty()) return std::unexpected(mw::runtimeError("No public key in actor"));
        ...
        if (existing && existing.value())
        {
            ...
            {
                user.shared_inbox = json_ld::getId(j["endpoints"], "sharedInbox");
            }
...
{
    // 1. Date Validation
    if (!req.has_header("Date")) return std::unexpected(mw::httpError(401, "Missing Date header"));
    ...
    {
        if (!req.has_header("Digest")) return std::unexpected(mw::httpError(401, "Missing Digest header"));
        ...
        if (digest_header.starts_with("SHA-256="))
        {
            ...
            if (!local_digest_bytes) return std::unexpected(mw::runtimeError("Hash failed"));
            std::string local_digest = mw::base64Encode(*local_digest_bytes);
            if (remote_digest != local_digest) return std::unexpected(mw::httpError(401, "Digest mismatch"));
        }
    }
    ...
    auto construct_comparison_string = [&](const std::string& headers) -> std::string {
        ...
        while (std::getline(ss, header_name, ' '))
        {
            ...
            {
                if (!req.has_header(header_name)) return ""; // Error
    ...
    {
        if (verify_with_key(user_res.value()->public_key)) return actor_uri;
        spdlog::info("Verification failed with cached key for {}, re-fetching...", actor_uri);
    }
